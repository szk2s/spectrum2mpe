...
function genSMFs(
  ...
  return new Promise((resolve) => {
    ...
    const { bpm, ppqn, pitchBendRange, defaultVelocity } = _.merge(
      defaultOption,
      _option
    );
    ...
    melodies.sort(
      (melodyA, melodyB) =>
        utils.average(melodyB.amps) - utils.average(melodyA.amps)
    );
    ...
    const tracks = bunchesOfMelodies.reduce(
      (tracks, oneBunchOfMelodies, trackIdx) => {
        console.log(
          'generating track ' +
            (trackIdx + 1) +
            ' (Total ' +
        ...
        track.add(
          utils.second2Tick(endTime, bpm, ppqn) + ppqn,
        ...
        switch (noteOnOff) {
          case 1:
            track.add(
              ...
              new JZZ.MIDI.noteOn(
                ch,
                melody.activeNoteNums[frameIdx],
                defaultVelocity
              )
            );
            break;
