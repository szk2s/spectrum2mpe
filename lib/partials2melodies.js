...
function partials2melodies(_partials) {
  return new Promise((resolve) => {
    ...
    const melodies = partials.map((partial) => {
      ...
      [melody.activeNoteNums, melody.deltaCents] = calcActiveNoteNums(
        partial.freqs,
        melody.noteOnOffs
      );
      return melody;
    ...
    function detectNoteOnNoteOffIndices(amps) {
      ...
      amps.forEach((amp, idx, amps) => {
        ...
        if (idx == 0) {
          if (amp > 0) {
            noteOnOffs.push(NOTE_ON);
          } else {
            noteOnOffs.push(NOTHING);
          }
        } else if (amp > 0 && prevAmp == 0) {
          noteOnOffs.push(NOTE_ON);
        } else if (amp == 0 && prevAmp > 0) {
          noteOnOffs.push(NOTE_OFF);
        } else {
          noteOnOffs.push(NOTHING);
        }
      });
    ...
    function calcActiveNoteNums(freqs, noteOnOffs) {
      ...
      const activeNoteNums = noteOnOffs.reduce(
        (activeNoteNums, noteOnOff, idx, noteOnOffs) => {
          // noteOnOff value is
          // noteOn = 1, noteOff = -1, keep = 0
          if (noteOnOff === 1) {
            activeNoteNums.push(closestNoteNums[idx]);
          } else if (noteOnOff === -1) {
            activeNoteNums.push(activeNoteNums[activeNoteNums.length - 1]);
          } else if (noteOnOff === 0) {
            if (noteOnOffs[idx - 1] == null || noteOnOffs[idx - 1] === -1) {
