    ...
    genSMFs(melodies, songName){
        return new Promise((resolve, reject) => {
            ...
            const tracks = bunchesOfMelodies.reduce((tracks, bunch, trackIdx) => { 
                ...
                if(totalTracks === 1){
                    track.add(0, JZZ.MIDI.smfSeqName(songName));
                }else{
                    track.add(0, JZZ.MIDI.smfSeqName(songName + '-' + (trackIdx+1)));
                }
                track.add(0, JZZ.MIDI.smfBPM(this.config.bpm));

                //add melody into track
                bunch.forEach((melody, melodyIdx) => {
                    // console.log('generating channel ' + (ch + 1) +  ' of track ' + (trackIdx + 1))
                    if(melodyIdx > 15){
                        throw new Error("bad melodyIdx");
                    }
                    this.addMelodyToTrack(melody, track, melodyIdx+1);
                });
