...
class S2M {
    ...
    async convertFromSpear(songName){
        let data;
        data = await this.txtImport(songName);
    ...
    txtImport(songName){
        return new Promise((resolve, reject) => {
            ...
            const data = partials.reduce((data, partial)=>{
                data.timecode.push(partial.timecode);
                data.freqs.push(partial.freqs);
                data.amps.push(partial.amps);
                return data;
            },{timecode : [], freqs : [], amps : []})
    ...
    parseText(text){
        ...
        const partials = textArray.reduce((partials, stringData, idx, textArray)=> {
