...
const extractPeakFreqs = (partials, targetPeaksNum = 15) => {
  ...
  frames.forEach((frame, idx, frames) => {
    console.log('marking frame ' + (idx + 1) + ' (total ' + frames.length + ' frames)');
...
function markPeaks(frame, targetPeaksNum = 15) {
  ...
  const { main: peakIDs, sub: subPeakIDs } = collectPeakIDs(points, targetPeaksNum);
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const subPeakPoints = subPeakIDs.map((id) => points.find((point) => point.id === id));
...
function collectPeakIDs(_points, targetNum = 1) {
  ...
  for (let i = 0; i < _points.length; i++) {
    ...
    _points = peakIDs.main.map((id) => _points.find((point) => id === point.id));
...
export default extractPeakFreqs;
